...
public class IBCAppModuleBasic: AppModuleBasic {
...
//    func (AppModuleBasic) RegisterLegacyAminoCodec(*codec.LegacyAmino) {}
    ...
    public func defaultGenesis() -> JSON? {
        let data = Codec.ibc.mustMarshalJSON(value: IBCGenesisState.default)
        return Codec.ibc.mustUnmarshalJSON(data: data)
    }

    // ValidateGenesis performs genesis state validation for the ibc module.
    public func validateGenesis(json: JSON) throws {
        let data = Codec.ibc.mustMarshalJSON(value: json)
        let state: IBCGenesisState
        
        do {
            state = try Codec.ibc.unmarshalJSON(data: data)
        } catch {
            throw CosmosError.wrap(
                error: error,
                description: "failed to unmarshal \(HostKeys.moduleName) genesis state"
            )
        }
